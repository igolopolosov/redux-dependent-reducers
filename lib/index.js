...
var DependentReducers = (function () {
    ...
    DependentReducers.prototype.combine = function (stateShape) {
        ...
        return function (state, action) {
            if (action.type === '@@INIT') {
                _this.allDependencies.forEach(function (dependency) {
                    var key = _this.stateIdToKey[dependency.id];
                    if (key) {
                        state = __assign({}, state, (_a = {}, _a[key] = dependency.getCurrentState(), _a));
                    }
                    var _a;
                });
            }
            else if (_this.reducersByActionType.hasOwnProperty(action.type)) {
